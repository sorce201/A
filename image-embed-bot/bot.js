...
// التعليقات لا تظهر داخل البطاقة إطلاقًا — فقط عدّادها على زر "عرض التعليقات"
// وعند الضغط عليه تظهر كرسالة خاصة (Ephemeral) للعضو فقط.
// بدون Thread وبدون رسائل جديدة تُرسل تحت — كل شيء داخل نفس الرسالة الواحدة.
...
function buildEmbed(messageKey, imageRef) {
  ...
  // التعليقات تظهر هنا داخل نفس البطاقة، فوق الصورة، مرئية للجميع فور إضافتها.
  ...
  try {
    if (interaction.isButton()) {
      ...
      if (action === 'view_comments') {
        // التعليقات أصلاً ظاهرة بالبطاقة فوق الصورة — هذا الزر يعرض لك القائمة الكاملة خاص لو تجاوزت الحد المعروض
        const comments = commentsStore.get(messageKey) || [];
    ...
    if (interaction.isModalSubmit()) {
      ...
      if (interaction.customId.startsWith('comment_modal:')) {
        ...
        // تحديث الإيمبد نفسه ليضم التعليق الجديد — يظهر للجميع فورًا فوق الصورة
        try {
